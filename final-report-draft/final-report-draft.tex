...
The running time of the integer linear program solved by GLPK \cite{GLPK} is given in seconds while the running time of our implementation are in nanoseconds. Given the precision of the running time of the GLPK\cite{GLPK} a logarithmic representation is impossible (0s values).
...
\subsection{Cellular Graph - variable numbers of colors available (higher than 4), each node has 4 possible colors}

The fact that the running time depends on the number of colors available for a fixed size of cellular graph is though to test because of the machine capacity. Indeed, the running time of the integer linear program and its need in memory can be very high.
We have compared the running time depending on the number of colors available for a cellular graph of size N=40.
The 
...
\caption{R studio result for a fixed size of $N=40$ (cf.Fig.\ref{cellgraphgeneration})}
\label{20size}
\end{figure}

\subsection{Cellular Graph - Generated Coloration}
By interest, we decided to compare the actual coloration of the same graph by the two methods. Obvisously most of time both coloration were different. However, for in the first case (4 colors available and each are available for each node) the coloration was exactly the same. A further work could be to prove that there is one unique solution in this case for cellular graph 4-coloration.
The scripts generated can be found in the git repository.
